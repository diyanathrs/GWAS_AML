#!/bin/bash
                            
#SBATCH --partition=defq,long                                                                      
#SBATCH --time=04:00:00                                                                                   
#SBATCH --nodes=1                                                                                         
#SBATCH --ntasks=1                                                                                        
#SBATCH --cpus-per-task=1                                                                                 
#SBATCH --mem-per-cpu=32G                                                                                  
#SBATCH --export=ALL                                                                                      
#SBATCH --mail-type=FAIL                                                                                  
#SBATCH --mail-user=d.s.ranasinghe2@newcastle.ac.uk                                                            
#SBATCH --array=13-13                                                                                     
##SBATCH --chdir=${rootpath}/association


rootpath=$(pwd)

#impvpath="${rootpath}/../impvRes"
#gwas="NCL12"

module purge
#module load R
module load QCTOOL/v2.0-rc8-foss-2019a 



# keep only controls not cases
#plink2 --bfile HRC_chr${SLURM_ARRAY_TASK_ID} --export bgen-1.2 --out merged_bgen/HRC_merged_chr${SLURM_ARRAY_TASK_ID} 

#qctool -g chr${SLURM_ARRAY_TASK_ID}_merged.bgen -snp-stats -osnp chr${SLURM_ARRAY_TASK_ID}_merged.info                                                             

../../../finemapping/bgen_v1.1.4-CentOS6.8-x86_64/bgenix -g chr${SLURM_ARRAY_TASK_ID}_merged.bgen -index
