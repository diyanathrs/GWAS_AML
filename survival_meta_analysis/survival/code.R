gen2dosageSurv <- function(dataChunk, phenofile, cen.col, time.col, ncpu, chrom,...){
    ...
    #argnames <- list(...) if (!("ncpu" %in% argnames)){ncpu=detectCores()} print(c('ncpu =', ncpu))
...
## check which allele is minor
    ...
    snp.info[AA.probs.sum >= BB.probs.sum, MAF :=mafB[AA.probs.sum >= BB.probs.sum]][AA.probs.sum < BB.probs.sum, 
MAF:=mafA[AA.probs.sum < BB.probs.sum]]
    setnames(snp.info, paste0("V", seq(1,6)), c("chr", "rsid", "snp", "posb37", "alleleA", "alleleB"))
    ## if sum(AA.probs.sum) >= sum(BB.probs.sum) then A is major allele and B is minor allele if above is TRUE, B dosage is 
    ## used, above is FALSE, A dosage is used dosages based on the minor allele
    ...
    if (length(snps.count2)>0) {
        ...
        res.out <- foreach(n=seq_along(snps.count2), .combine=rbind, .export="snps.count2") %dopar% {
            tryCatch(
            {
                surv.sub.1 <- data.table(.id=phenos1[[".id"]], status=phenos1[[cen.col]], time=phenos1[[time.col]], 
snp=dosageall.out[snps.count2[n], ])
                setnames(surv.sub.1, c("status", "time", "snp"), c(cen.col, time.col, snps.count2[n]))
                ## check if allele counts==0 in any one of study if (all(table(dosageall.out[snps.count2[n], ] !=0, 
                ## phenos1$.id)["TRUE", ] !=0) ){
            ...
            ## } else {
            ##    ahr.p <- NA ahrci <- NA ahr.n <- NA ahr.nevent <- NA
            ## }
...
samples <- fread("impvSubset/Finland_chr22subsetMOD.sample", header=TRUE)[-1, ] genfile <- 
file("impvSubset/Finland_chr22subset.gen", open="r") sapply(strsplit(readLines(genfile, n=1)," "), length) 
stopifnot(sapply(strsplit(readLines(genfile, n=1)," "), length)==(samples[,.N]*3+6)) close(genfile)
## the type of columns
cols <- rep("numeric", samples[, .N]*3+6 ) cols[c(1,2,3,5,6)] <- "character"
## read the data for a chunk
con <- file("impvSubset/Finland_chr22subset.gen", open="r") dataChunk <- read.table(con, nrows=2, skip=0, header=FALSE, 
colClasses=cols) phenofile <- 'association/AMLSurpheno_Finland_13Mar2019.txt' cen.col <- 'OSstatus' time.col <- 'OSdays' 
ncpu <- 1 chrom <- 22 kkk <- c("survival", "data.table", "doParallel") lapply(kkk, require, character.only=TRUE) 
gen2dosageSurv(dataChunk, phenofile, cen.col, time.col, ncpu, chrom)
