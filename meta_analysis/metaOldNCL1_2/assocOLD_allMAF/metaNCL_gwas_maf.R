...
snptest.cols <- c("rsid", "chromosome", "position", "alleleA", "alleleB", "frequentist_add_pvalue", "frequentist_add_beta_1", "frequentist_add_se_1", "info_score", "all_maf","cases_maf", "controls_maf")
...
# added by Dean to get different maf
print(maf)

chrom.res[all_maf <= maf,
                    ':=' (frequentist_add_beta_1=NA, frequentist_add_se_1=NA, frequentist_add_pvalue=NA)
                    ]

#chrom.res[controls_maf <= maf,
#                    ':=' (frequentist_add_beta_1=NA, frequentist_add_se_1=NA, frequentist_add_pvalue=NA)
#                    ]
...
if (length(exclude.out)!=0){
    write.table(chrom.res[rsid1 %chin% exclude.out, ],
                file=paste0(phenoprefix, "_",maf, dataprefix,"_excluded.out"),
...
write.table(chrom.res[!(SNP %chin% exclude.out),  c("SNP", "CHR", "BP", "A2", "A1", "BETA", "SE", "P"), with=FALSE],
            sep="\t",
            file=paste0(phenoprefix, "_MAF",maf, dataprefix,"_MAF_assoc.res"), quote=FALSE,
