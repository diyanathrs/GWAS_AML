...
## read meta res
chrom.res <- fread(paste0( "snps.meta"))
...
folder.lst <- fread(paste0(folderlist), header=FALSE)
   ...
   #             row.names=FALSE, col.names=TRUE)
    ...
    write.table(study.res, file=paste0(phenoprefix, "_NCL_", dataprefix, "_hitsByStudy.txt"), quote=FALSE, sep="\t",  row.names=FALSE, col.names=TRUE)                                        
