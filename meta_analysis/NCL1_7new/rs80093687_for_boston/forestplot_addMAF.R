...
for (n in seq_along(hits.snps)){
    ...
    pdf(paste0(phenoprefix, hits.snps[n], dataprefix, ".pdf"), onefile=TRUE, width=10, height=6)
    ...
    text(par("usr")[1]+ (par("usr")[2]-par("usr")[1])*0.14, (par("usr")[4]-par("usr")[3])*0.8 + par("usr")[3], "Info_score",cex=0.8)
