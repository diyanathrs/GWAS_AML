...
study.res[cases_maf <= 0.0095,
                    ':=' (frequentist_add_beta_1=NA, frequentist_add_se_1=NA, frequentist_add_pvalue=NA)
                    ]

study.res[controls_maf <= 0.0095,
                    ':=' (frequentist_add_beta_1=NA, frequentist_add_se_1=NA, frequentist_add_pvalue=NA)
                    ]
...
pdf(paste0(phenoprefix, "_NCL_", dataprefix, "_forest_addeaf.pdf"), onefile=TRUE, width=8, height=6)
...
for (n in seq_along(hits.snps)){
    ...
    text(par("usr")[1], (par("usr")[4]-par("usr")[3])*0.78 + par("usr")[3],"Study",pos=4)
    text(par("usr")[1]+ (par("usr")[2]-par("usr")[1])*0.25, (par("usr")[4]-par("usr")[3])*0.78 + par("usr")[3], "EAF (Case/Con)")
    text(par("usr")[1]+4.7*(par("usr")[2]-par("usr")[1])/5.8,  (par("usr")[4]-par("usr")[3])*0.78 + par("usr")[3],  "Weight(%)",pos=2)
    text(par("usr")[2], (par("usr")[4]-par("usr")[3])*0.78 + par("usr")[3] ,"OR [95% CI]", pos=2)
