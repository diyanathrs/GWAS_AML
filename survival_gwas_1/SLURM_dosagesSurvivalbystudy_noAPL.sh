...
datadir=${rootpath}/impvSubset_${subset}

# change file name of your master accordingly
phenofile="../association/AMLSurpheno_Finland_13Mar2019.txt"

assoc_dir=association_${subset}
mkdir -p ${assoc_dir}
...
declare -a cencol=('OSstatus' )
declare -a timecol=('OSdays' )
...
#SBATCH --array=2-2
#SBATCH --chdir=${rootpath}/${assoc_dir}
...
Rscript --vanilla ${rootpath}/HRbystudyFMSrun_rscriptMOD.R "${phenofile}" "${cencol[i]}" "${timecol[i]}" "${datadir}" "${study[j]}" "${prefix}"
