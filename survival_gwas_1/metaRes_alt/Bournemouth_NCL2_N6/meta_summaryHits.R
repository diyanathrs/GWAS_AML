...
dataprefix1 <- paste0(dataprefix, "N6")
...
if (chrom.res[P<10^-7, .N]!=0){
    ...
    setwd("./Hitsmeta")
    write.table(hits, file=paste0(phenoprefix, "_NCL_", dataprefix1, "_hits.lst"), quote=FALSE, sep="\t",
                row.names=FALSE, col.names=TRUE)
    ## folder list
    ## folder.lst <- fread("/home/nwl15/WORKING_DATA/HRCimpvData/metaNCL1_4/NCL1_4_folder.lst", header=FALSE)
    ## if (dataprefix=="gt"){
    ##     folder.lst <- folder.lst[1:3, ]
    ## } else {
    ##     folder.lst <- folder.lst[c(1:2, 4),]
    ## }
    folder.lst <- data.table(V1="/home/nwl15/WORKING_DATA/HRCimpvData/CLLbystudyFinalInfo90/ResultSummary",
                             V2=c("Bournemouth", "Cardiff", "Hull1", "Hull2", "Newcastle1", "Newcastle2")
                             )
    ## extract function
    extract.data <- function(x){
        ...
        for (n in seq_along(hits.chr)){
            out.res[[n]] <- readRDS(paste0(folder.lst[x, V1], "/", folder.lst[x, V2], "_", phenoprefix, "_", dataprefix, "_chrom", hits.chr[n], "_cleaned.rds"))[
    ...
    write.table(study.res, file=paste0(phenoprefix, "_NCL_", dataprefix1, "_hitsByStudy.txt"), quote=FALSE, sep="\t",
                row.names=FALSE, col.names=TRUE)
}                                       # 
