...
infoindex <- as.numeric(infoindex)
infoindex
...
chrom.res <- chrom.res[info >= infothre[infoindex], ]

mafthre   <- seq(0.01, 0.06, by=0.01)

png(paste0(phenoprefix, "_meta_" , dataprefix, "qq_HRCbyMAFinfo", infoindex, ".png"), type="cairo", width=8, height=6, units="in", res=150)
#qq.plot(chrom.res[, P])
...
mtext(paste0("studyMAF > 1% & present in 6 studies, info >=", infothre[infoindex]), side=3, outer=TRUE, cex=1)
