...
if (chrom.res[P<10^-6, .N]!=0){
    ...
    ## }
    folder.lst <- data.table(V1="/home/nwl15/WORKING_DATA/HRCimpvData/CLLbystudy/ResultSummary",
    ...
    extract.data <- function(x){
        ...
        for (n in seq_along(hits.chr)){
            out.res[[n]] <- readRDS(paste0(folder.lst[x, V1], "/", folder.lst[x, V2], "_", phenoprefix, "_", "CLL_finalQced4imp", "_chrom", hits.chr[n], "_cleaned.rds"))[
    ...
    write.table(study.res[studyMAF >= 0.01, ], file=paste0(phenoprefix, "_NCL_", dataprefix, "_hitsByStudy.txt"), quote=FALSE, sep="\t",
                row.names=FALSE, col.names=TRUE)
}                                       # 
