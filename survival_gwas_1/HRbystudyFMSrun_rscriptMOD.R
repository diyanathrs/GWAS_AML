...
dataprefix <- "CLL_OEEfinalQced4imp"


genfileName <- paste0(dataprefix, "_chr", chrom, "subset.gen.gz")
gensample <- paste0(dataprefix, "_chr", chrom, "subset.samples")
## phenofile <- "../phenos/CLL_Hull3pheno.txt"
...
genfile <- pipe(paste0("zcat ", data.dir, "/", genfileName), open="r")

sapply(strsplit(readLines(genfile, n=1)," "), length)
stopifnot(sapply(strsplit(readLines(genfile, n=1)," "), length)==(samples[, .N]*3+5))
close(genfile)

## the type of columns
cols <- rep("numeric", samples[, .N]*3+5 )
##cols[c(1,2,3,5,6)] <- "character"
cols[c(1, 2, 4, 5)] <- "character"

## read the data for a chunk
## con <- file(paste0(data.dir, "/", genfileName), open="r")
con <- pipe(paste0("zcat ", data.dir, "/", genfileName), open="r")
...
dataChunk$V2 <- ifelse(dataChunk$V2==".", paste0(chrom, ":", dataChunk$V3), dataChunk$V2)
...
if (any(duplicated(dataChunk$V2))){
    ...
    dataChunk$V2 <- ifelse(dataChunk$V2 %in% dup.ids,
                           paste0(dataChunk$V2, "_", dataChunk$V4, "_", dataChunk$V5),
                           dataChunk$V2)
}
...
## dosage
start.AA <- 1 + 3*(1-1) + 5
end.AA <- 1 + 3*(samples[, .N] -1) + 5
AA.columns <- seq(start.AA, end.AA, by=3)

start.AB <- 2 + 3*(1-1) + 5
end.AB <- 2 + 3*(samples[, .N] -1) + 5
AB.columns <- seq(start.AB, end.AB, by=3)

start.BB <- 3 + 3*(1-1) + 5
end.BB <- 3 + 3*(samples[, .N] -1) + 5
...
snp.info <- setDT(dataChunk[, 1:5])
snp.info[AA.probs.sum >= BB.probs.sum, minorAllele:=V5][AA.probs.sum < BB.probs.sum, minorAllele:=V4]
...
setnames(snp.info, paste0("V", seq(1,5)), c("chr", "rsid", "posb37", "alleleA", "alleleB"))
