...
pdf(file=paste0(snps[n], pheno,"_Figure2_2_visAbs.pdf"), width=8, height=4.5)
...
forest(fixed, slab=plot.sub[, study], refline=1, transf=exp, 
       ...
       cex = 1.2, colout = '#528B8B', border = '#CD1076', col = '#CD1076', pch = 19, lwd=2)

addpoly(random, row=-0.5, transf=exp, cex = 1.2,col = '#CD1076', border = '#CD1076',
...
addpoly(random, row=-0.5, transf=exp, cex = 1.2, border = '#CD1076', col = '#CD1076', 
        mlab=paste0("Random-effect (p=", format(random$pval,digits=3,scientific = TRUE, nsmall=3), ")"))
op <- par(cex=1.2, font=2)
#par(op)
#text(par("usr")[1], (par("usr")[4]-par("usr")[3])*0.81 + par("usr")[3],"GWAS", pos=4)
...
pdf(file=paste0(snps[n], pheno,"_Figure2_2_visAbs.pdf"), width=8, height=5, onefile = F)
par(mar=c(4,4,1,2))
pos <- c(-4.9, -3.1, -1.6, -0.2)
forest(fixed, slab=plot.sub[, study], refline=1, transf=exp, 
       xlim=c(-7.5, 6),
       at=c(0.5,1,2, 4),
       #clim=c(0.5, 3.5),
       ...
       mlab=paste0("Fixed-effect (p=", format(fixed$pval,digits=3,scientific = TRUE, nsmall=3), ")"),
       xlab=bquote(paste(.(snpname), .(fixedtext3)[het],.(fixedtext2)," I"^{2},.(randomI),sep="")),
       cex = 1.2,  colout = '#528B8B', border = '#CD1076', col = '#CD1076', pch = 19)
...
op <- par(cex=1.2, font=2)
#par(op)
text(par("usr")[1], (par("usr")[4]-par("usr")[3])*0.8 + par("usr")[3],"GWAS", pos=4)
text(pos, 6.7, c('No \n case/con',"Info \n score","Effect \n allele","EAF \n case/con" ),adj = c(0.5,0.5))
text(par("usr")[2], (par("usr")[4]-par("usr")[3])*0.8 + par("usr")[3] ,"OR [95% CI]", pos=2)
dev.off()
...
#############################
####### do vis Abs forest for sur snps ###
#############################
...
# only keep snps on int
hrc.rsid <- hrc.rsid[hrc.rsid$ID %in% c(snps,ch.snps),]
head(hrc.rsid)
...
i <- 'rs4665765'
  ...
  pdf(paste0('os_forestNew/',snpname, "_forest_forVisAbs.pdf"), onefile=F, width=8, height=5.2)
  par(mar=c(4,4,2,2))
  pos <- c(-3.2, -1.8, -0.7, 0.1)
  
  forest(fixed, slab=plot.sub[, source], refline=1, transf=exp, showweight=F, xlim = c(-5, 4), alim = c(0,2), at = c(0,1,2),
         ilab=i.annot, ilab.xpos = pos, header = c("GWAS", "HR [95% CI]"),
         mlab=paste0("Fixed-effect (p=", format(fixed$pval, digits=3, scientific = TRUE,nsmall=3), ")"),
         xlab=bquote(paste(.(snpname), .(fixedtext3)[het],.(fixedtext2)," I"^{2},.(randomI),sep="")), cex = 1.2, 
         colout = '#528B8B', border = '#CD1076', col = '#CD1076', pch = 19, lwd=2)
  ...
  addpoly(random, row=-0.5, transf=exp, mlab=paste0("Random-effect (p=", format(random$pval, digits=3,scientific = TRUE,nsmall=3), ")"),
          border = '#CD1076', col = '#CD1076')
  addpoly(collider.corr, row=-1.5, transf=exp, mlab=paste0("Adjusted for index bias (p=", format(snp.collider$p.adj, digits=3,
                                              scientific = TRUE,nsmall=3), ")"),border = '#CD1076', col = '#CD1076')
  text(pos,10, c('No case/\n events','Info \n score' ,'Effect \nallele','EAF'), font=2, cex=1.2)
